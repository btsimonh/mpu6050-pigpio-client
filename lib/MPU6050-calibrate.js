...
function addCalibrate(MPU6050Inst) {
    ...
    // get avg values by reading fifo for loops x 50ms
    // note: data includes gravity.
    // if accRange and gyroRange are unset (undefined), then 2g/250dps will be used.
    // set hw_test_enable to enable hw test offset.
    MPU6050Inst.get_avg_values = async function( accRange, gyroRange, packets, hw_test_enable ) {
        packets = packets || 500;
        let loops = 1;
    ...
    MPU6050Inst.get_hw_scales = async function( loops ) {
        let org_offsets = await this.getHWOffsets();
        let org_values = await this.get_avg_values(undefined, undefined, loops);
        ...
        let mod_values = await this.get_avg_values(undefined, undefined, loops);
    ...
    // pure test function I used to ge tthe empirical hardware scale LSB
    MPU6050Inst.testScale = async function() {
        ...
        let num = 1;
        for (let i = 0; i < num; i++){
            let hw_values = await MPU6050.get_hw_scales(80);
